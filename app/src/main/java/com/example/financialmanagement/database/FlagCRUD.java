...
public class FlagCRUD {
    ...
    public FlagCRUD(Context context,String DBNAME,int VERSION)// 定义构造函数
    {
     ...
     * @param mFlag
     */
    public void update(Flag mFlag) {
        db = dbUserDataHelper.getWritableDatabase();// 初始化SQLiteDatabase对象
        db.execSQL("update tb_flag set flag = ? where _id = ?", new Object[] {
                mFlag.getFlag(), mFlag.getid() });// 执行修改便签信息操作
    }
    ...
    public void detele(Integer... ids) {
        if (ids.length > 0)// 判断是否存在要删除的id
        {
            ...
            db = dbUserDataHelper.getWritableDatabase();// 创建SQLiteDatabase对象
            // 执行删除便签信息操作
    ...
    public List<Flag> getScrollData(int start, int count) {
        ...
        {
            // 将遍历到的便签信息添加到集合中
            lisTb_flags.add(new Flag(cursor.getInt(cursor
