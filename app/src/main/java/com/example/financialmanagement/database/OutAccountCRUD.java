...
/**
 * Created by wangdy11 on 2017/7/17.
 */
...
public class OutAccountCRUD {
    private DBUserData dbUserDataHelper;// 创建DBOpenHelper对象
    private SQLiteDatabase db;// 创建SQLiteDatabase对象

    public OutAccountCRUD(Context context,String DBNAME,int VERSION)// 定义构造函数
    {
     ...
     * @param mOutAccount
     */
    public void add(OutAccount mOutAccount) {
        ...
        db.execSQL(
                ...
                new Object[] { mOutAccount.getid(), mOutAccount.getMoney(),
                        mOutAccount.getTime(), mOutAccount.getType(),
                        mOutAccount.getAddress(), mOutAccount.getMark() });
    }
     ...
     * @param mOutAccount
     */
    public void update(OutAccount mOutAccount) {
        db = dbUserDataHelper.getWritableDatabase();// 初始化SQLiteDatabase对象
        // 执行修改支出信息操作
        db.execSQL(
                "update tb_outaccount set money = ?,time = ?,type = ?,address = ?,mark = ? where _id = ?",
                new Object[] { mOutAccount.getMoney(),
                        mOutAccount.getTime(), mOutAccount.getType(),
                        mOutAccount.getAddress(), mOutAccount.getMark(),
                        mOutAccount.getid() });
    }
     ...
     * @return
     */
    public List<OutAccount> getScrollData(int start, int count) {
        List<OutAccount> tb_outaccount = new ArrayList<OutAccount>();// 创建集合对象
        db = dbUserDataHelper.getWritableDatabase();// 初始化SQLiteDatabase对象
        // 获取所有支出信息
        ...
        {
            // 将遍历到的支出信息添加到集合中
            tb_outaccount.add(new OutAccount(cursor.getInt(cursor
